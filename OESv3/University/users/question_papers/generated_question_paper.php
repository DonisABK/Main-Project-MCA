...
if(isset($_POST['id']) && $_POST['id'] > 0){
    $qry = $conn->query("SELECT q.*,CONCAT(cc.name,' - ',c.name) as `class` from `tbl_question_paper_list` q inner join tbl_class_list c on q.class_id = c.id inner join tbl_course_list cc on c.course_id = cc.id where q.id = '{$_POST['id']}' ");
    if($qry->num_rows > 0){
        ...
        <div class="card-body">
            <div id="outprint">
                ...
                <hr>
                    ...
                    <?php 
                    $i = 1;
                    $questions = $conn->query("SELECT * from `tbl_question_list` where question_paper_id = '{$id}' and `type` = 1 order by RAND() limit {$single}");
                    ...
                    <div class="question-item mb-3">
                        ...
                        <div class="mx-2 mb-3">
                            <div class="row">
                                <?php 
                                $options = $conn->query("SELECT * FROM tbl_choice_list where question_id = '{$row['id']}'");
                ...
                <hr>
                    ...
                    <?php 
                    $i = 1;
                    $questions = $conn->query("SELECT * from `tbl_question_list` where question_paper_id = '{$id}' and `type` = 2 order by RAND() limit {$multiple}");
                    while($row= $questions->fetch_assoc()):
                    ?>
                    <div class="question-item mb-3">
                        <div class="d-flex w-100 mb-1">
                ...
                <hr>
                    ...
                    <?php 
                    $i = 1;
                    $questions = $conn->query("SELECT * from `tbl_question_list` where question_paper_id = '{$id}' and `type` = 3 order by RAND() limit {$text}");
                    while($row= $questions->fetch_assoc()):
                    ?>
                    <div class="question-item mb-3">
                        <div class="d-flex w-100 mb-1">
