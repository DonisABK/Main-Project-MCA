	...
	<form action="" id="type-form">
		...
		<div class="form-group">
			...
			<select name="class_id" id="class_id" class="form-control form-control-sm rounded-0 select2" required>
				...
				<?php 
				$class = $conn->query("SELECT c.*,concat(cc.name,' - ', c.name) as `class` FROM `tbl_class_list` c inner join tbl_course_list cc on c.course_id = cc.id where c.delete_flag = '0' and c.`status` = 1 and c.user_id = '{$_settings->userdata('id')}'  ".(isset($class_id) ? " or c.id = '{$class_id}'" : "")."  order by cc.`name` asc,c.`name` asc");
