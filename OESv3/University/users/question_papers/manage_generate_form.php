...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `tbl_question_paper_list` where id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
$single_max = $conn->query("SELECT * FROM tbl_question_list where question_paper_id = '{$id}' and `type` = 1 ")->num_rows;
$multiple_max = $conn->query("SELECT * FROM tbl_question_list where question_paper_id = '{$id}' and `type` = 2 ")->num_rows;
$text_max = $conn->query("SELECT * FROM tbl_question_list where question_paper_id = '{$id}' and `type` = 3 ")->num_rows;
?>
