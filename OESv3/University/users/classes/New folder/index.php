        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped table-bordered" id="list">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT c.*,cc.name as `course`, cc.stream as  `stream` from `tbl_class_list` c inner join `tbl_course_list` cc on c.course_id = cc.id where c.delete_flag = 0 and c.user_id = '{$_settings->userdata('id')}' order by cc.`name` asc,c.`name` asc ");
