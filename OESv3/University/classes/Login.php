...
class Login extends DBConnection {
	...
	public function login(){
		...
		return json_encode(array('status'=>'incorrect','last_qry'=>"SELECT * from tbl_users where username = '$username' and password = md5('$password') "));
		}
	}
	...
	function login_ruser(){
		extract($_POST);
		$stmt = $this->conn->prepare("SELECT * from tbl_registered_user_list where email = ? and `password` = ? and delete_flag = 0 ");
		$password = md5($password);
