        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped table-bordered" id="list">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT q.*,CONCAT(cc.name,' - ',c.name) as `class` from `tbl_question_paper_list` q inner join tbl_class_list c on q.class_id = c.id inner join tbl_course_list cc on c.course_id = cc.id where q.delete_flag = 0 and q.user_id = '{$_settings->userdata('id')}' order by CONCAT(cc.name,' - ',c.name) asc, q.`title` asc ");
...
<script>
	$(document).ready(function(){
		...
		$('.scrutiny').click(function(){
			uni_modal("<i class='fa fa-edit'></i> Scrutiny","question_papers/view_question_answerskey.php")
		})
		$('.inviligator').click(function(){
			uni_modal("<i class='fa fa-edit'></i> Inviligator","question_papers/list_students_answer_paper.php")
		})
