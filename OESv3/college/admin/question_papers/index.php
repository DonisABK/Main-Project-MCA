        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped table-bordered" id="list">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT q.*,CONCAT(cc.name,' - ',c.name) as `class` from `tbl_question_paper_list` q inner join tbl_class_list c on q.class_id = c.id inner join tbl_course_list cc on c.course_id = cc.id where q.delete_flag = 0  order by CONCAT(cc.name,' - ',c.name) asc, q.`title` asc ");
						while($row = $qry->fetch_assoc()):
							$uemail = $conn->query("SELECT email from tbl_registered_user_list where id = '{$row['user_id']}'")->fetch_array()[0];
					?>
						<tr>
